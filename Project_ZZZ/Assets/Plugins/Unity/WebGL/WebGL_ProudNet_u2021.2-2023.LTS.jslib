...
var WebGL = {
	...
	// === Added for Unity 2021.2+ : allocate/ALLOC_STACK 대체용 문자열 헬퍼 ===
	...
	$SetEnvironment: function() {
		...
		NetClient.prototype.connect = function (param) {
			...
			this.socketWrapper.socket.onclose = function (closeEvent) {
				...
				{
					// B-version: Module['dynCall_*'] 사용
					Module['dynCall_viiii'](
						callbacks.onLeaveServer,
						_this.instanceID,
						_this.errorType,
						_this.detailType,
						allocCString(_this.reason + "[" + disconnReason + "]")
					);
				}
			};
		...
		NetClient.prototype.processMessage_ProudNetLayer = function (msg, remote) {
			...
			switch (msgType) {
				...
				case MessageType.UserMessage:
					// B-version: Module['dynCall_*']
					Module['dynCall_viii'](
						callbacks.onUserMessage,
						this.instanceID,
						remote,
						allocCString(Uint8ArrayToBase64(msg.getArray().subarray(1)))
					);
					break;
		...
		NetClient.prototype.processMessage_ConnectServerTimedout = function (msg) {
			// B-version
			Module['dynCall_viiii'](
				callbacks.onJoinServerComplete,
				this.instanceID,
				allocCString(ErrorType.ConnectServerTimeout + "|" + 0),
				allocCString("Remote detected no receive."),
				allocCString("")
			);
		};
		...
		NetClient.prototype.processMessage_NotifyServerConnectSuccess = function (msg) {
			...
			// B-version
			Module['dynCall_vii'](callbacks.onHostIDIssued, this.instanceID, this.localHostID);
			
			Module['dynCall_viiii'](
				callbacks.onJoinServerComplete,
				this.instanceID, 
				allocCString(ErrorType.Ok + "|" + 0), 
				allocCString(""), 
				allocCString(Uint8ArrayToBase64(userData.getArray()))
			);
		};
		NetClient.prototype.processMessage_NotifyProtocolVersionMismatch = function (msg) {
			...
			Module['dynCall_viiii'](
				callbacks.onJoinServerComplete,
				this.instanceID,
				allocCString(ErrorType.ProtocolVersionMismatch + "|" + 0),
				allocCString("NPVM1"),
				allocCString("")
			);
		};
		NetClient.prototype.processMessage_NotifyLicenseMismatch = function (msg) {
			...
			Module['dynCall_viiii'](
				callbacks.onJoinServerComplete,
				this.instanceID,
				allocCString(ErrorType.InvalidLicense + "|" + 0),
				allocCString("NLM1"),
				allocCString("")
			);
		};
		NetClient.prototype.processMessage_NotifyServerDeniedConnection = function (msg) {
			...
			Module['dynCall_viiii'](
				callbacks.onJoinServerComplete,
				this.instanceID,
				allocCString(ErrorType.NotifyServerDeniedConnection + "|" + 0),
				allocCString("NPDC"),
				allocCString("")
			);
		};
		...
		NetS2CStub.prototype.ProcessReceivedMessage = function (msg) {
			...
			switch (msgType) {
				...
				case ProudS2C.Rmi_ReliablePong:
					...
					// B-version
					Module['dynCall_vi'](callbacks.onSynchronizeServerTime, this.client.instanceID);
					break;
...
//		'$deepObjCopy',
	...
	CreateJSClient: function(url, protocolVersion, userData, internalVersion) {
		...
		// B-version: Pointer_stringify -> UTF8ToString
		...
		param.userData.setArray(UTF8ToString(userData) == "" ? new Uint8Array(new ArrayBuffer(0)) : base64ToUint8Array(UTF8ToString(userData)));
